...
export default function AdminTwoWheelerPage() {
  ...
  const load = useCallback(async () => {
    try {
      const url = filter ? `/api/admin/2wheeler?status=${encodeURIComponent(filter)}` : "/api/admin/2wheeler";
      const data = await api<TwoWheelerRequest[]>(url);
      setRequests(data);
    } catch {
      setRequests([]);
    } finally {
  ...
  return (
    <div className="space-y-4">
      <div>
        <h1 className="text-2xl font-bold text-slate-900 dark:text-white">2 Wheeler Requests</h1>
                    ...
                    <Button
                      ...
                      onClick={async () => {
                        if (confirm("Delete this request?")) {
                          try {
                            await api(`/api/admin/2wheeler/${row.id}`, { method: "DELETE" });
                            await load();
                          } catch (err) {
                            alert((err as Error).message);
                          }
