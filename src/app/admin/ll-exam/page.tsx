...
export default function AdminLlExamPage() {
  ...
  const load = useCallback(async () => {
    try {
      const url = filter ? `/api/admin/ll-exam-request?status=${encodeURIComponent(filter)}` : "/api/admin/ll-exam-request";
      const data = await api<LlExamRequest[]>(url);
      setRequests(data);
    } catch {
      setRequests([]);
    } finally {
  ...
  return (
    <div className="space-y-4">
      <div>
        <h1 className="text-2xl font-bold text-slate-900 dark:text-white">LL Exam Requests</h1>
        <p className="text-slate-500 dark:text-gray-400">
          Manage all learning licence exam requests submitted by retailers.
        </p>
      </div>
                    ...
                    <Button
                      ...
                      onClick={async () => {
                        if (confirm("Delete this request?")) {
                          try {
                            await api(`/api/admin/ll-exam-request/${row.id}`, { method: "DELETE" });
                            await load();
                          } catch (err) {
                            alert((err as Error).message);
                          }
