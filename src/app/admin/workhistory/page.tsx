...
export default function AdminWorkHistoryPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const result = await api<HistoryItem[]>(`/api/admin/workhistory?${params.toString()}`);
      setData(result);
    } catch {
      setData([]);
    } finally {
  ...
  return (
    <div className="space-y-4">
      <div>
        <h1 className="text-2xl font-bold text-slate-900 dark:text-white">Work History</h1>
        <p className="text-slate-500 dark:text-gray-400">All retailer work history records.</p>
      </div>
            ...
            <Table>
              <TableHeader>
                <TableRow className="bg-slate-50 dark:bg-white/5">
                  ...
                  <TableHead className="text-slate-700 dark:text-gray-300">Service ID</TableHead>
                ...
                {loading ? (
                  ...
                  data.map((row) => (
                    <TableRow key={row.id} className="hover:bg-slate-50 dark:hover:bg-white/5">
                      ...
                      <TableCell className="text-slate-700 dark:text-gray-300">{row.service_id ?? "-"}</TableCell>
