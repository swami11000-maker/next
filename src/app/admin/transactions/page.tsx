  ...
  const load = useCallback(
    async (pageNumber: number = 1) => {
      try {
        ...
        }>(`/api/admin/transitions?${params.toString()}`);
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <Button variant="outline" size="sm" onClick={handleRefresh} disabled={fetching} className="w-fit gap-2">
        ...
        <CardContent className="pt-6">
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div className="relative w-full sm:w-80">
              ...
              <Input className="pl-8" placeholder="Search service, order, remark..." value={search} onChange={(e) => setSearch(e.target.value)} />
            </div>

            <select className="h-9 rounded-md border border-slate-200 bg-white px-3 text-sm dark:border-slate-700 dark:bg-slate-900" value={status} onChange={(e) => setStatus(e.target.value)}>
          ...
          <div className="mt-3 flex items-center justify-between text-sm text-slate-600 dark:text-slate-400">
            <span>
              Showing <strong className="text-slate-900 dark:text-white">{data.length}</strong> of <strong className="text-slate-900 dark:text-white">{pagination.total}</strong> records
              {fetching && (
            ...
            <Table>
              ...
              <TableBody>
                {initialLoading ? (
                  ...
                  <TableRow>
                    <TableCell colSpan={9} className="py-12 text-center text-red-500">
                      {error}
                    </TableCell>
                  </TableRow>
                ) : data.length === 0 ? (
                  ...
                  data.map((row) => (
                    <TableRow key={row.id} className="hover:bg-slate-50 dark:hover:bg-white/5">
                      ...
                      <TableCell className="text-slate-700 capitalize dark:text-gray-300">{row.tranfer_type}</TableCell>
                      <TableCell>
                        <span className={`inline-flex rounded-full px-2 py-1 text-xs font-medium ${STATUS_COLORS[row.status] || 'bg-gray-100 text-gray-700'}`}>{row.status}</span>
                      </TableCell>
                      <TableCell className="text-slate-700 dark:text-gray-300">{row.date_time ? formatIndianDateTime(row.date_time) : '-'}</TableCell>
                    </TableRow>
                  ))
                )}
              </TableBody>
