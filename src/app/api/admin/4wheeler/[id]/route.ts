...
import { z } from "zod";
import { runQuery, runMutation, runTransaction, SqlParam, FOURWHEELER_SAFE_COLUMNS } from "@/lib/auth";
...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const rows = await runQuery<FourWheelerRequest[]>(`SELECT ${RETAILER_SELECT} FROM \`4wheeler\` WHERE id = ? LIMIT 1`, [Number(id)]);
    ...
    console.error("Get 4wheeler error:", error);
...
export async function PUT(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (result.data.status !== undefined) {
      updates.push("`status` = ?");
      values.push(result.data.status as FourWheelerStatus);
    ...
    await runTransaction(async (conn) => {
      ...
      const sql = `UPDATE \`4wheeler\` SET ${updates.join(", ")} WHERE id = ? LIMIT 1`;
    ...
    const updated = await runQuery<FourWheelerRequest[]>(`SELECT ${RETAILER_SELECT} FROM \`4wheeler\` WHERE id = ? LIMIT 1`, [Number(id)]);
...
export async function DELETE(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    await runMutation("DELETE FROM `4wheeler` WHERE id = ? LIMIT 1", [Number(id)]);

    return NextResponse.json({ message: "Request deleted successfully" });
  } catch (error) {
    console.error("Delete 4wheeler error:", error);
    return NextResponse.json({ message: "Internal server error" }, { status: 500 });
  }
}
