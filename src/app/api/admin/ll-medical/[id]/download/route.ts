...
export async function GET(
  ...
  try {
    ...
    const rows = await runQuery<any[]>(
      `SELECT ${SELECT_DOC} FROM \`ll_medical\` WHERE id = ? LIMIT 1`,
      [Number(id)]
    );
    ...
    return new NextResponse(buffer, {
      headers: {
        ...
        "Content-Disposition": `attachment; filename="ll-medical-${rows[0].id}.pdf"`,
        "Content-Length": buffer.length.toString(),
      },
    });
  } catch (error) {
    console.error("Download ll_medical doc error:", error);
