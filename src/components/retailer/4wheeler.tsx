...
export default function FourWheelerReq() {
  ...
  const onSubmit = async (data: FormValues) => {
    ...
    const loadingToastId = toast.loading("Uploading documents...", {
      description: "Please wait while we process your 4 wheeler vehicle request.",
    });

    try {
      const [frontside, backside] = await Promise.all([fileToBase64(data.frontSidePhoto), fileToBase64(data.backSidePhoto)]);

      const response = await apiFetch("/api/4wheeler", {
        method: "POST",
  ...
  const handleReset = () => {
    ...
    toast.info("Form cleared", {
      description: "All entered 4 wheeler vehicle information has been removed.",
    });
  };
      ...
      <div className="mx-auto w-full max-w-4xl">
        ...
        >
          ...
          <h1 className="text-3xl font-bold tracking-tight text-slate-900 dark:text-white sm:text-4xl lg:text-5xl">
            Vehicle <span className="text-[#ff3800]">4 Wheeler</span>
          </h1>

          <p className="mx-auto mt-2 max-w-xl text-sm text-slate-500 dark:text-gray-400 sm:text-base">Upload your vehicle 4 Wheeler photos and submit your request</p>
        </motion.div>
            ...
            <CardHeader className="px-4 pb-6 sm:px-6">
              <div className="flex flex-col gap-4 border-b border-slate-100 pb-5 dark:border-white/10 sm:flex-row sm:items-center sm:justify-between">
                <CardTitle className="flex items-center gap-2 text-lg font-semibold text-slate-800 dark:text-white sm:text-xl">
                  ...
                  <span>Enter 4 Wheeler Details</span>
                </CardTitle>

                <ServiceChargeCard charge={retailer?.["4wheeler_fee"] ?? 0} serviceName="4 Wheeler" className="w-full sm:w-auto sm:max-w-none" />
              </div>
            </CardHeader>
                      ...
                      render={({ field }) => (
                        <FormItem>
                          <FormLabel className="flex items-center gap-2 text-slate-700 dark:text-gray-300">
                            ...
                            <span>4 Wheeler Vehicle Number</span>
