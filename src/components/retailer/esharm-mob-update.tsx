...
import { Search, Loader2, AlertCircle, ArrowRight, CreditCard, RefreshCcw, CalendarDays, CheckCircle2, Smartphone, Receipt, BadgeCheck, User, FileText, ArrowLeftRight, Hash } from "lucide-react";
...
const formSchema = z.object({
  uid: z
    .string()
    .length(12, { message: "UID must be exactly 12 digits" })
    .regex(/^\d{12}$/, { message: "Enter a valid 12-digit UID" }),
...
export default function EsharmMobileUpdatePage() {
  ...
  const onSubmit = async (data: FormValues) => {
    ...
    try {
      ...
      const resData = apiResult.data || (apiResult as unknown as ApiResponse);

      if (!resData.success || resData.StatusCode !== 100) {
        const errMsg = resData.message || apiResult.message || "Mobile update failed.";
        setError(errMsg);
        return;
      }
      ...
      <motion.div initial="hidden" animate="visible" variants={containerVariants} className="max-w-2xl mx-auto">
        ...
        <motion.div variants={itemVariants} className="text-center mb-10">
          <div className="mx-auto mb-4 flex h-14 w-14 items-center justify-center rounded-2xl bg-[#ff3800]/10">
            <Smartphone className="h-7 w-7 text-[#ff3800]" />
          </div>

          <h1 className="text-4xl sm:text-5xl font-bold text-slate-900 dark:text-white mb-2 tracking-tight">
            E-Sharm <span className="text-[#ff3800]">Mobile Update</span>
          </h1>
          <p className="text-slate-500 dark:text-gray-400 text-lg">Update mobile number linked to E-Sharm card</p>
        </motion.div>
              ...
              <div className="flex flex-col gap-4 border-b border-slate-100 pb-5 dark:border-white/10 sm:flex-row sm:items-center sm:justify-between">
                <CardTitle className="flex items-center gap-2 text-lg font-semibold text-slate-800 dark:text-white sm:text-xl">
                  <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-[#ff3800]/10">
                    <ArrowLeftRight className="h-5 w-5 text-[#ff3800]" />
                  </span>
                  <span>Mobile Number Update</span>
                </CardTitle>

                <ServiceChargeCard charge={retailer?.esharm_mob_update_fee ?? 0} serviceName="E-Sharm Mobile Update" className="w-full sm:w-auto sm:max-w-none" />
              </div>
            </CardHeader>
                ...
                <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-5">
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        <FormLabel className="text-slate-700 dark:text-gray-300 flex items-center gap-2">
                          <Smartphone className="h-4 w-4 text-[#ff3800]" />
                          New Mobile Number
                          <span className="text-[#ff3800]">*</span>
                        </FormLabel>
                        <FormControl>
                        ...
                        <p className="text-xs text-slate-400 dark:text-gray-500">Example: 9876543210</p>
                        <FormMessage className="text-[#ff3800]" />
                      </FormItem>
                    )}
                  />
              ...
              <Alert variant="destructive" className="border-red-300 dark:border-red-500/50 bg-red-50 dark:bg-red-500/10 text-red-700 dark:text-red-400 rounded-2xl">
                ...
                <AlertDescription>
                  <div className="flex flex-col gap-1">
                    <span>{error}</span>
                    {searchedUid && <span className="text-xs opacity-80">UID: {searchedUid}</span>}
                  </div>
                </AlertDescription>
              </Alert>
            </motion.div>
            ...
            <DialogHeader className="px-6 pt-5 pb-0 space-y-2">
              <div className="flex items-start justify-between gap-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <DialogTitle className="text-xl font-bold text-black">Mobile Updated</DialogTitle>
                    <DialogDescription className="text-sm text-gray-500">E-Sharm mobile number changed successfully</DialogDescription>
                  </div>
                </div>
            ...
            <div className="px-6 overflow-y-auto max-h-[50vh]">
              <div className="space-y-4 pb-2">
                <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="rounded-xl border border-orange-200 bg-orange-50 p-4 text-center">
                  <p className="text-sm font-semibold text-orange-800">{result?.message || result?.data?.message || "Update completed"}</p>
                </motion.div>

                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <DetailItem icon={User} label="Name" value={result?.name || result?.data?.data?.name} highlight />
                  <DetailItem icon={Hash} label="UAN Number" value={result?.uan_no || result?.data?.data?.uan_no} mono />
                  <DetailItem icon={CreditCard} label="UID Number" value={result?.uid_no || result?.data?.data?.uid_no} mono />
                  <DetailItem icon={Smartphone} label="Old Mobile" value={result?.old_mobile || result?.data?.data?.old_mobile} />
                  <DetailItem icon={Smartphone} label="New Mobile" value={result?.new_mobile || result?.data?.data?.new_mobile} highlight />
                  <DetailItem icon={Receipt} label="Application No" value={result?.application_no || result?.data?.application_no} mono />
                  <DetailItem icon={BadgeCheck} label="Transaction ID" value={result?.transaction_id || result?.data?.transaction_id} mono />
                  <DetailItem icon={FileText} label="Status Code" value={String(result?.statusCode || result?.data?.StatusCode)} />
